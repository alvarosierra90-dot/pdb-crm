...
export default function StackingPlan({ initBuildings, onCountChange, onOwnersChange, onBuildingsChange, activoPropietario='', extraOwners=[], extraTenants=[], onAddOwner, onAddTenant, onConvertToTenant, onTenantClick, extraOfertas=[], initView='principal', defaultSupPlantaTipo, defaultLabel='', allowCreate=true, noDataMessage=null }) {
  ...
  const insertFloorAt = (idx) => {
    ...
    // Derive a sensible floor ID from the neighbour above (idx-1) or below (idx)
    const floors = edif.floors
    const above = floors[idx - 1]   // floor that will be directly above the new one
    const below = floors[idx]        // floor that will be directly below
  ...
  return (
    <div>
              ...
              return (
                ...
                >
                  ...
                  <div style={{padding:'4px 4px 4px 0',display:'flex',flexDirection:'column',gap:4}}>
                    {/* Track: mismo ancho siempre. Fill coloreado proporcional al largo (superficie) */}
                    <div style={{height:barH,borderRadius:4,overflow:'hidden',background:isTgt?'var(--accent-lt)':isSel?'#dbeafe':'#e5e7eb',border:`1px solid ${isSel||isTgt?'var(--accent)':'var(--border)'}`,display:'flex',alignItems:'stretch'}}>
                      {floor.principal.length===0 ? (
                        <div style={{flex:1,display:'flex',alignItems:'center',justifyContent:'center',fontSize:10,color:isSel?'var(--accent)':isTgt?'var(--accent)':'var(--text4)',fontWeight:isTgt||isSel?600:400}}>
                          {isTgt?'⬇ Soltar uso aquí':isSel?'✓ Seleccionada — arrastra un uso':'Clic para seleccionar · arrastra un uso'}
                        </div>
                      ) : (
                        <div style={{width:barW,flexShrink:0,display:'flex',alignItems:'stretch',gap:2,overflow:'hidden'}}>
                          ...
                          {avail>0 && (
                            <div style={{flex:1,minWidth:14,background:'#f1f5f9',border:`1px dashed var(--border)`,borderRadius:2,display:'flex',alignItems:'center',justifyContent:'center',fontSize:9,color:'var(--text4)'}}>
                ...
                return (
                  <div key={floor.id}
                    ...
                    <div style={{display:'flex',flexDirection:'column',gap:3,padding:'5px 4px 5px 0'}}>
                      ...
                      <div style={{height:barH,borderRadius:4,overflow:'hidden',background:isTgt?'var(--accent-lt)':'#e5e7eb',border:`1px solid ${isTgt?'var(--accent)':'var(--border)'}`,display:'flex',alignItems:'stretch'}}>
                        {isEmpty ? (
                          <div style={{flex:1,display:'flex',alignItems:'center',justifyContent:'center',fontSize:10,color:isTgt?'var(--accent)':'var(--text4)',gap:5}}>
                ...
                return (
                  <div key={floor.id}
                    ...
                    <div style={{display:'flex',flexDirection:'column',gap:3,padding:'5px 4px 5px 0'}}>
                      ...
                      <div style={{height:barH,borderRadius:4,overflow:'hidden',background:dropWarning===floor.id?'#fff1f2':isTgt?'var(--accent-lt)':'#e5e7eb',border:`1px solid ${dropWarning===floor.id?'#fca5a5':isTgt?'var(--accent)':'var(--border)'}`,display:'flex',alignItems:'stretch'}}>
                        {isEmpty ? (
                          <div style={{flex:1,display:'flex',alignItems:'center',justifyContent:'center',fontSize:10,color:isTgt?'var(--accent)':'var(--text4)',gap:5}}>
      ...
      {convertConfirm&&(
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,.45)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:1000}} onClick={()=>setConvertConfirm(null)}>
          <div style={{background:'var(--surface)',borderRadius:'var(--r2)',padding:22,width:360,boxShadow:'0 8px 32px rgba(0,0,0,.22)',display:'flex',flexDirection:'column',gap:14}} onClick={e=>e.stopPropagation()}>
            ...
            <div style={{fontSize:12,color:'var(--text2)',lineHeight:1.6}}>
              ...
              ¿Quieres registrarlo como nuevo arrendatario?
            </div>
            <div style={{fontSize:10,color:'var(--text3)',background:'var(--gray-lt)',padding:'8px 10px',borderRadius:'var(--r)',lineHeight:1.5}}>
              Se abrirá la ficha completa de arrendatario con los datos de la oferta pre-rellenados.
            </div>
            <div style={{display:'flex',gap:8,justifyContent:'flex-end'}}>
              ...
              <button onClick={()=>{
                  if(onConvertToTenant) onConvertToTenant(convertConfirm.unit, convertConfirm.floorId, convertConfirm.idx)
                  else removeArrUnit(convertConfirm.floorId,convertConfirm.idx)
                  setConvertConfirm(null)
                }}
                style={{padding:'7px 16px',background:'var(--accent)',color:'#fff',border:'none',borderRadius:5,fontSize:11,cursor:'pointer',fontFamily:'inherit',fontWeight:600}}>
                Sí, crear arrendatario →
              </button>
            </div>
          </div>
        </div>
