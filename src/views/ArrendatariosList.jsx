...
export default function ArrendatariosList() {
  ...
  const preFiltered = ARRENDATARIOS.filter(a => {
  ...
  const totalSup    = ARRENDATARIOS.reduce((s,a)=>s+a.superficie,0)
  const rentaAnual  = ARRENDATARIOS.reduce((s,a)=>s+a.superficie*a.renta_media*12,0)
  const nActivos    = ARRENDATARIOS.filter(a=>a.estado==='Activo').length
  const nAlertas    = ARRENDATARIOS.filter(a=>a.estado==='Próximo a vencimiento').length
  const nNeg        = ARRENDATARIOS.filter(a=>a.estado==='En negociación').length
  ...
  const cell = (a) => ({
    ...
    arrendatario: <td key="arrendatario"><div className="asset-link">{a.arrendatario}</div><div className="asset-sub">{a.sector}</div></td>,
    ...
    superficie:   <td key="superficie" className="mono">{a.superficie.toLocaleString('es-ES')} m²</td>,
    renta_media:  <td key="renta_media" className="mono">{a.renta_media.toFixed(2)} €/m²</td>,
    ...
    area:         <td key="area"><span className="tag tag-blue" style={{fontSize:9}}>{a.area}</span></td>,
    ...
    trimestre:    <td key="trimestre"><span className="tag tag-teal" style={{fontSize:9}}>{a.trimestre}</span></td>,
    ...
    _act:         <td key="_act"><div className="ra-cell"><button className="ra p" onClick={e=>{e.stopPropagation();navigate('ficha-arrendatario')}}>Ver</button></div></td>,
  })

  return (
    <div style={{display:'flex',flexDirection:'column',flex:1,overflow:'hidden'}}>

      {/* Alertas break option */}
      {ARRENDATARIOS.filter(a=>a.estado==='Próximo a vencimiento').length > 0 && (
      ...
      <div className="kpi-strip" style={{gridTemplateColumns:'repeat(5,1fr)'}}>
        <div className="ks"><div className="ks-lbl">Total arrendatarios</div><div className="ks-val">{ARRENDATARIOS.length}</div></div>
        <div className="ks"><div className="ks-lbl">Activos</div><div className="ks-val green">{nActivos}</div></div>
        ...
        <div style={{padding:'10px 16px',background:'var(--gray-lt)',borderBottom:'1px solid var(--border)',display:'flex',flexWrap:'wrap',gap:10,alignItems:'flex-end'}}>
          <Field label="Estado">
            <select className="fsel" value={af.estado} onChange={e=>setAf(p=>({...p,estado:e.target.value}))}>
              <option value="">Todos</option><option>Activo</option><option>Próximo a vencimiento</option><option>En negociación</option><option>Renovado</option><option>Finalizado</option>
            </select>
          </Field>
        ...
        <table className="main-tbl">
          <thead>
            <tr>{visibleCols.map(c=>
              ...
              <ColHeader key={c.id} col={c} sorts={sorts} filters={filters} setSort={setSort} setFilter={setFilter} clearFilter={clearFilter} allRows={ARRENDATARIOS}/>
            )}</tr>
          </thead>
          <tbody>
            {result.map(a=>(
              <tr key={a.id} onClick={()=>navigate('ficha-arrendatario')} style={{cursor:'pointer',opacity:a.estado==='Finalizado'?.6:1}}>
