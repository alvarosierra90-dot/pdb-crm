...
const ZONAS = [
  { id:'MAD-M30', zona:'M-30 / Distrito Centro', subzona:'M-30', provincia:'Madrid', uso:'Oficinas', activos_n:2, activos:['P.E Avalon','Parque Empresarial Norte'], stock:58156, disponible:13024, ocupacion:78.9, ofertas_activas:3, takeup_ytd:8500, ops_ytd:4, renta_media:13.5, renta_min:10.5, renta_max:16.8, Disponibilidad:22.4 },
  { id:'ALC-A1', zona:'Alcobendas / A-1', subzona:'Corredor A-1', provincia:'Madrid', uso:'Oficinas', activos_n:1, activos:['Albatros'], stock:53944, disponible:13486, ocupacion:75.0, ofertas_activas:2, takeup_ytd:5200, ops_ytd:2, renta_media:12.5, renta_min:12.0, renta_max:14.5, Disponibilidad:25.0 },
  { id:'BCN-22AT', zona:'22@ / Diagonal', subzona:'22@', provincia:'Barcelona', uso:'Oficinas', activos_n:2, activos:['Edificio Diagonal 95','Torre Glòries'], stock:28300, disponible:1176, ocupacion:95.8, ofertas_activas:1, takeup_ytd:3200, ops_ytd:3, renta_media:25.0, renta_min:22.0, renta_max:28.0, Disponibilidad:4.2 },
  { id:'MAD-GET', zona:'Getafe / Corredor A-4', subzona:'Getafe Industrial', provincia:'Madrid', uso:'Logístico', activos_n:1, activos:['Park Logístico Getafe'], stock:24000, disponible:960, ocupacion:96.0, ofertas_activas:1, takeup_ytd:12000, ops_ytd:1, renta_media:6.8, renta_min:6.5, renta_max:7.2, Disponibilidad:4.0 },
  { id:'MAD-LEG', zona:'Leganés / Sur Madrid', subzona:'Sur M-40', provincia:'Madrid', uso:'Retail', activos_n:1, activos:['Centro Comercial Parquesur'], stock:42000, disponible:3780, ocupacion:91.0, ofertas_activas:2, takeup_ytd:2100, ops_ytd:2, renta_media:18.0, renta_min:15.0, renta_max:22.0, Disponibilidad:9.0 },
  { id:'VLC-MES', zona:'Mestalla / Valencia', subzona:'Centro Valencia', provincia:'Valencia', uso:'Oficinas', activos_n:1, activos:['Torre Europa Valencia'], stock:7600, disponible:1292, ocupacion:83.0, ofertas_activas:1, takeup_ytd:900, ops_ytd:1, renta_media:14.5, renta_min:13.0, renta_max:16.0, Disponibilidad:17.0 },
]
      ...
      <div className="list-toolbar">
        ...
        <div style={{marginLeft:'auto',display:'flex',gap:6}}>
          ...
          <ExportMenu getConfig={() => ({
            ...
            subtitle: `${result.length} zonas · Savills PDB`,
            coverMetrics: [
              { label: 'Total zonas', value: result.length },
              { label: 'Stock total (m²)', value: result.reduce((s,z)=>s+z.stock,0).toLocaleString('es-ES') },
              { label: 'Disponible (m²)', value: result.reduce((s,z)=>s+z.disponible,0).toLocaleString('es-ES') },
            ],
            sections: [{
              title: 'Listado de zonas',
              type: 'table',
              headers: ['ID','Zona','Provincia','Uso','Stock m²','Disponible','Ocupación','Renta media'],
              rows: result.map(z=>[z.id, z.zona, z.provincia, z.uso, z.stock.toLocaleString('es-ES'), z.disponible.toLocaleString('es-ES'), `${z.ocupacion}%`, `${z.renta_media} €/m²`]),
            }],
          })} />
        </div>
      </div>
